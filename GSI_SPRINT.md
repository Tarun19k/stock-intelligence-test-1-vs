...
| **QA-D3-01** | **P1: `_calc_roe()` returns 0.37% for INFY.NS (calculated) vs 32.68% (`returnOnEquity` field) — likely USD/INR currency mismatch. `netIncomeToCommon` appears in USD for USD-reporting NSE stocks; `bookValue × sharesOutstanding` are in INR. Fix: when `returnOnEquity` field is available and calculated value diverges >10pp, prefer `returnOnEquity` OR add currency detection. Confirmed only if CEO Screener.in cross-check validates ~32% actual ROE.** | Low | quant_audit_session_024 | Policy 5 (data coherence) |
