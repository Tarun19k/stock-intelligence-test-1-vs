...
def render_plan(items: list[dict], sprint_name: str,
    ...
    print(f"     {YELLOW}IMPORTANT: Always run /new-session WITHOUT proxy vars (Step 1 above).{RESET}")
    print(f"     {YELLOW}           /new-session needs tool use — Groq models do not support it.{RESET}")
    print(f"     {YELLOW}           Only set proxy vars AFTER /new-session completes.{RESET}")
