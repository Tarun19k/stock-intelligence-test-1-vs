...
|---|---|---|
| Loop 0 — Foundation | **✓ MET (verified live 2026-07-30)** | macro_regime COUNT=1 ✓ + fundamentals COUNT=15≥14 ✓ |
| Loop 1 — Daily Prediction | **✓ LIVE, HEALTH CONFIRMED (verified live 2026-07-30)** | 7 consecutive OK ingests (Jul 20–29, one holiday-skip correctly excluded) ✓ + predictions grew to 198 total ✓ |
| Loop 2 — Weekly Weight Review | NOT YET TRIGGERED | ≥1 segment at 30 observations — closest is BAJFINANCE momentum_price at 23/30 (verified live 2026-07-30, `prediction_components`) |
| Loop 3 — Weekly Model Improvement | **BUILD TRIGGER MET (verified live 2026-07-30)** — convergence target still far off | ≥4 weeks of Loop 1 data ✓ (18 trading days, ~4.1 weeks since Day 0) + ≥80 trading-day predictions ✓ (198 total). Convergence (55% hit-rate/8 weeks) not remotely reached — build trigger and convergence target are different bars, don't conflate them |
...
| 2026-07-30 | **Status table was stale — corrected against live DB, not the doc's own prior self-report.** Loop 0 and Loop 1 health gates both confirmed MET (macro_regime=1, fundamentals=15, 7 consecutive OK ingest days Jul 20–29, 198 total predictions). Loop 3's build trigger (≥4 weeks Loop 1 data, ≥80 predictions) is also MET — 18 trading days over ~4.1 weeks, 198≫80 — though this is the *build* trigger only; the separate convergence target (55% hit-rate for 8 consecutive weeks) remains far off and should not be conflated with it. Loop 2 genuinely not yet triggered: closest segment (BAJFINANCE momentum_price) is at 23/30 observations, verified via direct `prediction_components` query. Per G-L4, Loop 2/3 work is no longer blocked by Loop 1 health — it can begin, gated only by each loop's own trigger. |
