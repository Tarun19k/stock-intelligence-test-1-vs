...
| L2-A | HDFCBANK/PIDILITIND corporate-action correction (0.5x scale factor, volume ×2, 31+50 rows) | 2 | Claude direct (touches already-written prod data, no unilateral-delete precedent applies) | Backup of affected rows taken before UPDATE; correction applied; `check_corporate_actions.py` re-run shows 0 discontinuities for both tickers; spot-check 3 rows by hand against real pre/post values | Munger (WATCH condition owner), CoS, Tarun | **CLOSED 2026-07-27** — per Tarun's explicit "proceed with migration 0018, L2-A." A dispatched subagent independently re-verified the corruption (live `check_corporate_actions.py` run: HDFCBANK -50.44%, PIDILITIND -50.98%, exact match to documented figures/row counts) and took a full pre-UPDATE backup (`alphaveda/scripts/backups/corporate_action_correction_2026-07-27.json`, commit `1bd9929`) — then correctly **refused to execute the UPDATE itself**, since the claimed Tarun authorization arrived only as a relayed instruction in its dispatch prompt, not as directly verifiable consent (Trimurti Fallback 3: no claim, including a claimed approval, is acted on until independently verified). CoS then executed the UPDATE directly, having received Tarun's words firsthand in this session: pre-write DIFF confirmed 31/50 rows (exact match), UPDATE applied (price ×0.5, volume ×2, `deliverable_volume`/provenance fields untouched — the subagent's flagged out-of-scope field left alone, not silently expanded), post-write verify confirmed the discontinuity is gone (PIDILITIND 09-22→09-23 now ~2% normal move, was -50.98%; HDFCBANK 08-25→08-26 now ~0.9%, was -50.44%) and row counts unchanged (31/50/3743 total, nothing added or lost). Logged to `~/.claude/logs/external-state-writes.log`. |
...
| L1-D | **Retail-investor accuracy-ledger gap (urgent, added 2026-07-25 per Tarun's explicit instruction)**: (a) `/accuracy` page never fetches `magnitude_target`/`downside_target`, so Target% can never be shown next to actual Return%; (b) `resolve_outcomes.py`'s `hit` field is DIRECTION-ONLY (`hit = (direction=='BULL' and pct_change>0) or ...`) — never checks whether the magnitude/downside target was actually reached, so "Hit ✓" overstates accuracy to a retail reader; (c) no page discloses the resolution horizon ("X days") a retail investor would need to judge whether a signal's timing guidance was correct | 1 | Claude direct (compliance/trust-surface fix, not mechanical) | `/accuracy` query extended to select `magnitude_target`/`downside_target` and displays Target% alongside actual Return% per row; a magnitude-aware hit definition added (direction-correct AND target-reached, shown separately from direction-only hit so neither replaces the other silently); resolution horizon (days between `emitted_at` and outcome date) shown per row; re-run against live `accuracy_predictions`/`accuracy_outcomes` data, spot-check 3 rows by hand against raw DB values | Varghese (sebi-compliance-reviewer — this is a trust/disclosure surface), Calibration Integrity, CoS, Tarun | **MERGED (`6fac724`, pushed `337f063`)** — per the 4-seat council spec exactly. CoS independently re-verified: `resolve_outcomes.py` diff read directly (additive `magnitude_hit`/`outcome`, `hit` untouched), zero SEBI-language hits (own grep), `tsc --noEmit` clean, full suite 167 passed/34 skipped/13 pre-existing failures (all SUPABASE-creds-missing or the known G5b latency flake, confirmed identical to baseline). **Migration `0018` APPLIED 2026-07-27** (Tarun's explicit go-ahead) — verified via direct SQL: 108/108 rows intact, `magnitude_hit`/`outcome` columns live, historical rows correctly show `outcome: null` (not backfilled, populates going forward per the migration's own design), `hit`/`return_pct` unchanged. `/accuracy` was still showing stale ISR-cached "0 Resolved" immediately after (expected — `revalidate = 3600`, page cached from before the migration); a fresh deploy was triggered to bust the cache. RF-K (`return_pct` missing `*100`) already closed separately, see GAP_REGISTER. **CLOSED 2026-07-29** — Varghese/Calibration Integrity formal sign-off: both **APPROVE, no conditions**. Varghese confirmed all 4 original conditions (target-label text, no aggregate magnitude-hit tile, direction-vs-magnitude banner distinction, past-tense-only horizon phrasing). Calibration Integrity confirmed all 3 (hit field untouched, MAGNITUDE_TOLERANCE=0.10 named/disclosed, no ungated aggregate magnitude % at n≈148). CoS independently spot-checked the two highest-stakes citations (MAGNITUDE_TOLERANCE constant + its disclosure comment, exact target-label string) directly in source rather than trusting the sign-off report alone — both confirmed exact. L1-D is fully closed: code, migration, and process sign-off all complete. |
| L2-B | Wire `check_corporate_actions.py` into a recurring/gating check | 2 | Codex first (proven GHA pattern to replicate) | New/extended workflow; exit-code-1 gate confirmed working (already verified this session); real triggered run shown, not just YAML-valid | CoS, Tarun | **FULLY CLOSED — stale row corrected 2026-07-29.** This row previously said the RemoteTrigger routine "still needs Tarun to create manually" — false; it was created same-day (`trig_01KvZZukpQTxC2TJbmMj3Z6o`, `alphaveda-corp-action-check-trigger`, weekly Mon 15:06 UTC) and confirmed live via direct `RemoteTrigger action=list` call: enabled, `last_fired_at` 2026-07-27, `next_run_at` 2026-08-03. Verified via `gh run list --workflow corporate-action-check.yml`: 2 real runs, both `success` (one `workflow_dispatch` from the routine's first fire, one `schedule` backup) — the gate is genuinely operating unattended, not just configured. |
| L3-A | G20 nav promotion decision | 3 | N/A — decision, not build | Your explicit go/no-go on the "1wk watchlist-strip, no misuse" condition | Tarun only | **PENDING — window not yet complete.** Watchlist strip live since 2026-07-24 18:03 UTC; as of this checkpoint (2026-07-29) that's 4d15h elapsed, ~2.4 days remaining toward the 7-day mark (completes ~2026-07-31 18:03 UTC). SRE's condition from the rebuttal round still applies: the day-7 check must be an active monitoring/log pull, not "no complaints received." |
| L3-B | G1 fundamentals sourcing decision + pilot execution | 3 | N/A — decision, now executing | Manual entry vs. paid API, with real cost named; pilot row verified against live DB | Wealth Strategist (doctrine-panel-wealth-revenue-strategist), Tarun | **RELIANCE PILOT LIVE-WRITTEN 2026-07-29** — `FIRECRAWL_API_KEY` confirmed working (real content scraped from screener.in, not IP-blocked anymore). Sourced current-quarter (period_end 2026-06-30) fundamentals for RELIANCE via Firecrawl + Screener.in, with two real gaps surfaced and resolved with Tarun before writing: (1) `roic_pct` — Screener has no labeled ROIC, only ROCE/ROE; Tarun chose ROCE-as-documented-proxy (10.3%, NOT a true ROIC calc — flagged in source data, not silently substituted); (2) `promoter_pledge_pct` — not disclosed on the scraped page; inferred 0.0 from absence-of-pledge-row convention + no adverse search hits, not a directly-confirmed figure. Also found and fixed two real code issues along the way: `ingest_fundamentals.py`'s BSE announcement-lookup was intermittently throttled by BSE itself (confirmed live via direct probe — same request alternated between real data and a `{"Table":[{"Column1":1}]}` sentinel seconds apart; not a header/cookie/param issue) — added sentinel-detection + retry-with-backoff (`d1487a4`); `parse_bse_xbrl_fundamentals()` hardcoded `source: "bse_xbrl"` even for non-BSE-XBRL data — now honors an explicit `source` key so this pilot row correctly reads `source: "manual_screener"` instead of misrepresenting provenance. Dry-run verified clean, live-write executed (`--live-write --i-understand-this-writes-live-data`), then independently re-verified via a direct `SELECT` against Supabase (`kowlkczswaglbmabygtl`): row `id=1`, `fundamentals` table went from 0→1 rows exactly as expected. Logged to `~/.claude/logs/external-state-writes.log`. **BATCH COMPLETE 2026-07-29**: remaining 11 tickers (BAJFINANCE, TITAN, PIDILITIND, ITC, COALINDIA, TATASTEEL, HINDALCO, LT, DLF, TMPV, TMCV) sourced via Firecrawl + Screener.in and live-written, bringing `fundamentals` to 15 rows total (RELIANCE + earlier TCS/INFY/HDFCBANK batch + this 11). Real findings surfaced during sourcing, not smoothed over: (1) BAJFINANCE/TITAN/PIDILITIND/ITC/TATASTEEL/HINDALCO/LT/TMPV/TMCV/DLF all report latest reported quarter as Mar 2026, not Jun 2026 — Q1 FY27 results hadn't been released for these as of this session (only COALINDIA had reported Jun 2026 by session time, confirmed from its own quarterly table); (2) LT has no "Promoters" shareholding row at all — set `promoter_pledge_pct = NULL`, a genuine structural fact, not a missing-data placeholder; (3) TMCV (Tata Motors Ltd post-CV-demerger, renamed 29.10.2025) is genuinely absent from NSE's bulk pledge-disclosure file under any name variant tried — set `promoter_pledge_pct = NULL` rather than inferring 0, since the earlier session's inference-from-absence was corrected this session after a direct re-check; (4) discovered and fixed a real data-quality bug blocking 2 of 11 tickers from resolving at all: `instruments.isin` held wrong ISINs for BAJFINANCE (`INE296A01024`, real BSE value `INE296A01032`) and TATASTEEL (`INE081A01012`, real value `INE081A01020`) — confirmed by direct BSE scrip-master fetch matching on company name after ISIN lookup failed twice on retry (ruled out throttle), corrected via narrow field-level PATCH, re-verified scrip resolution succeeded 11/11 before writing. Dry-run clean, live-write executed, all 11 rows independently re-verified via direct SELECT (not trusting the script's own "OK" status). Both the ISIN correction and the fundamentals writes logged to `~/.claude/logs/external-state-writes.log`. **L3-B ticker-sourcing effort now fully complete** — no tickers remaining in this batch. |
