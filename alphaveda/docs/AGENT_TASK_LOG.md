...
| L3-B | G1 fundamentals sourcing decision | 3 | N/A — decision | Manual entry vs. paid API, with real cost named | Wealth Strategist (doctrine-panel-wealth-revenue-strategist), Tarun | **STALE ROW, CORRECTED 2026-07-29** — this said "PENDING — Wealth Strategist not yet asked," which was false; caught during a housekeeping checkpoint status-integrity pass (same class of gap `status-claim-integrity-auditor` was built for). **Actually DECIDED 2026-07-27** (see `GAP_REGISTER.md` G1 entry): manual quarterly entry over paid API, real Wealth Strategist pricing cited. Pipeline fixed and merged (`7bccf96`/`b0ed324`). **Execution (not decision) is what's still open**: the first real pilot ticker's data hasn't been sourced yet — blocked across two sessions on Tavily quota exhaustion, then a Firecrawl MCP registration gap (fixed via restart), then a Firecrawl free-tier IP block (needs a real `FIRECRAWL_API_KEY` from firecrawl.dev + one more restart to take effect). Fastest unblock remains: Tarun pastes real figures for 1-2 tickers directly. |
