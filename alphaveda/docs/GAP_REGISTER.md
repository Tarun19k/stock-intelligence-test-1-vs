...
| RF-H | `naturalFrequency()` phrasing ("right ~X in Y times") looks like a real historical tally but is a synthetic fraction-approximation of a confidence score | OPEN — found 2026-07-19 | `lexicon.ts:313` (`naturalFrequency`) takes a raw 0-100 confidence % and finds the closest simple fraction — it has no connection to actual resolved-outcome counts. A user reasonably reads "wrong about 1 in 1 times" as "we tried once and missed," which is false. Needs a wording decision (not a bug fix) — pending Tarun sign-off on replacement copy. |
