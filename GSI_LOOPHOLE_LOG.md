...
| Module-level cache used for multiple periods without period key | `_ticker_cache` stores OHLCV data per ticker regardless of `period`. A 3mo batch call that finds a 5d entry in cache serves it as "fresh" — the consumer's `len(df) < 10` guard then fires and shows a permanent loading state. Found in `_render_global_signals()` (v5.37.1 hotfix). Rule: any module-level cache that can serve multiple period lengths MUST include period in the cache key or fresh-serve check. | Manual review: check `_ticker_cache_period.get(s) == period` guard exists in `_yf_batch_download` fresh-serve path |
