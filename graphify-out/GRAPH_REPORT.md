...
- `DataType` --uses--> `TestDataContractBATCH`  [INFERRED]
  data_manager.py → tests/test_data_manager_m2.py
...
Cohesion: 0.08
Nodes (29): Breadcrumb: Current Week > India > Nifty 50, Refresh Data Button, Weekly Returns All Stocks Bar Chart, Market Filter (India), Search Filter (Company Name or Symbol), Nifty 50 Group, India Market, 49 Stocks Tracked Label (+21 more)
...
Cohesion: 0.12
Nodes (6): CacheManager, Bounded LRU in-memory cache (L2).      Deployment note: Streamlit-specific.  Sin, Returns DataResult if the entry exists (FRESH or STALE), None if not found., Store result.  No-op if status == UNAVAILABLE.         Evicts the LRU entry if a, Remove all cache entries for this ticker across all DataTypes., _make_result()

### Community 25 - "Community 25"
Cohesion: 0.18
Nodes (15): DataContract, DataResult, DataType, Wire-level shape validator for each DataType.      Validates that data received, Every DataManager response is a DataResult.     data is None if and only if stat, ResultStatus, SourceTag, Unit tests for DataManager M2 — CacheManager and DataContract.  Run: pytest test (+7 more)
...
Cohesion: 0.13
Nodes (23): Candlestick Chart, MACD (Daily) Chart, Price Chart with Bollinger Bands, RSI (14) Chart, Volume Chart, Chart Date Range: Nov 2025 to Apr 2026, Bollinger Bands Indicator, MACD (Daily) (+15 more)
...
## Knowledge Gaps
- **902 isolated node(s):** `R-ZIP · KI-014: Re-read packaged zip from disk and run full suite.     Catches f`, `Return the forecast history dict.     Primary store: st.session_state[_SS_KEY].`, `Persist the forecast history dict.     Always writes to session_state (works on`, `Record a new forecast entry (one per ticker per day).     simulation: optional d`, `Check all pending forecasts whose due date has passed and mark them     resolved` (+897 more)
...
- **Why does `Exception` connect `Community 55` to `Community 4`, `Community 36`, `Community 40`, `Community 49`, `Community 87`, `Community 94`?**
  _High betweenness centrality (0.070) - this node is a cross-community bridge._
- **Why does `get_supabase_client()` connect `Community 100` to `Community 36`, `Community 70`, `Community 40`, `Community 73`, `Community 74`, `Community 82`, `Community 83`, `Community 118`, `Community 55`, `Community 88`, `Community 93`, `Community 62`, `Community 95`?**
