...
## Corpus Check
- 156 files · ~1,392,686 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2285 nodes · 3369 edges · 164 communities detected
- Extraction: 87% EXTRACTED · 13% INFERRED · 0% AMBIGUOUS · INFERRED: 452 edges (avg confidence: 0.78)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `2df29645`
...
Cohesion: 0.05
Nodes (40): ABC, CustomLogger, CommercialLicenseError, DataProvider, DataProvider ABC — every OHLCV data source must subclass this.  CommercialLicens, Raised when accessing a personal-use-only source under a commercial licence., Exception, ApprovalLayer (+32 more)
...
Cohesion: 0.08
Nodes (18): LanguageToggle(), Lex(), LexOrRaw(), ProbabilityFrame(), isCommercial(), isPersonalContext(), GlossaryProvider(), useGlossary() (+10 more)
...
Cohesion: 0.19
Nodes (12): E2: consecutive BEAR emits exceed bucket-specific threshold.      Only counts em, should_exit_e2(), Phase 4 — optimizer.py tests., E2: long_term bucket needs 7 consecutive bears — 5 must not trigger., E2 UNCERTAINTY: doubles the threshold before firing., E2: exit fires when consecutive bears reach bucket threshold with sufficient con, E2: emits below confidence floor do not count — exit must not fire., test_exit_e2_bucket_threshold() (+4 more)
...
Cohesion: 0.18
Nodes (8): emit_signal(), Signal emit engine — full pipeline: arbitrate → discount → calibrate.  Pipeline, DB orchestrator: load context → build signal → emit → write accuracy_predictions, Imran (SRA): stale or missing ingest_status rows must surface as visible warning, If last_run > 1 day ago → amber banner shown (Imran SRA condition)., Imran (SRA): stale or missing ingest_status rows must surface as visible warning, Full emit pipeline (arbitration → discount → calibrate → kelly) ≤ 800ms., TestImranConditions
...
Cohesion: 0.22
Nodes (3): DataContract, Wire-level shape validator for each DataType.      Validates that data received, TestDataContractLIVE
...
Cohesion: 0.25
Nodes (4): DataResult, Returns DataResult if the entry exists (FRESH or STALE), None if not found., Every DataManager response is a DataResult.     data is None if and only if stat, TestDataContractINFO
...
Cohesion: 0.33
Nodes (6): E1: position drifted outside Kelly band.      Triggers when |current - kelly_tar, should_exit_e1(), E1: position that has drifted 60% away from Kelly target triggers exit., E1: position within tolerance band must not trigger exit., test_exit_e1(), test_exit_e1_within_band_no_exit()
...
- **Why does `Exception` connect `Community 7` to `Community 37`, `Community 4`, `Community 29`?**
  _High betweenness centrality (0.058) - this node is a cross-community bridge._
- **Why does `get_supabase_client()` connect `Community 47` to `Community 64`, `Community 37`, `Community 70`, `Community 7`, `Community 72`, `Community 73`, `Community 85`, `Community 88`, `Community 29`, `Community 63`?**
  _High betweenness centrality (0.047) - this node is a cross-community bridge._
- **Why does `_set_rate_limited()` connect `Community 4` to `Community 7`?**
