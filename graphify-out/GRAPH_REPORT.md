# Graph Report - stock-intelligence-test-1-vs  (2026-07-31)

## Corpus Check
- 196 files · ~1,480,487 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2575 nodes · 3806 edges · 183 communities detected
- Extraction: 87% EXTRACTED · 13% INFERRED · 0% AMBIGUOUS · INFERRED: 502 edges (avg confidence: 0.79)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `2c1a13c0`
...
3. `PATCH()` - 23 edges
4. `get_supabase_client()` - 23 edges
5. `main()` - 22 edges
6. `Left Sidebar Navigation` - 22 edges
7. `kelly_position_size()` - 21 edges
8. `Index Performance This Week` - 21 edges
9. `derive_cycle_phase()` - 20 edges
10. `ok()` - 19 edges
...
- `get_health_stats()` --calls--> `_tab_app_health()`  [INFERRED]
  market_data.py → pages/observability.py

## Communities (204 total, 45 thin omitted)
...
Cohesion: 0.05
Nodes (54): _build_isin_to_scripcode(), _fetch_scrip_master(), _find_latest_result_filing(), _http_get_json(), _is_throttle_sentinel(), _jittered_delay(), _map_parsed_to_fundamentals_row(), _probe_announcement_page() (+46 more)

### Community 3 - "Community 3"
Cohesion: 0.04
Nodes (58): Refresh Data Button, App Version v5.36, Refresh Data Button, Commodities & Global Inflation, Compare Tab, Open Dashboard Button CTA, Start Analysing Free Right Now CTA, Dark Theme UI (+50 more)
...
Cohesion: 0.05
Nodes (37): Phase 6 — G0 gate. All 10 criteria. Run criterion 10 (seed) first. Requires live, Criterion 6: SEBI disclaimer contains required substance., Criterion 7: Kelly returns 0 when downside_target is None., Criterion 7: Kelly returns 0 when downside_target is None., Criterion 8: Kelly returns non-zero rupee with valid downside_target., Criterion 8: Kelly returns non-zero rupee with valid downside_target., Criterion 8: Kelly returns non-zero rupee with valid downside_target., Criterion 9: disclaimer text passes substance check — non-occlusion is manual. (+29 more)

### Community 12 - "Community 12"
Cohesion: 0.09
Nodes (33): _get_dev_token(), _inline_compliance_check(), _market_status_rows(), _parse_audit_counts(), _parse_compliance_output(), _parse_loophole_log(), _parse_risk_counts(), _parse_risk_register() (+25 more)

### Community 13 - "Community 13"
Cohesion: 0.13
Nodes (15): CacheManager, DataResult, Bounded LRU in-memory cache (L2).      Deployment note: Streamlit-specific.  Sin, Returns DataResult if the entry exists (FRESH or STALE), None if not found., Store result.  No-op if status == UNAVAILABLE.         Evicts the LRU entry if a, Remove all cache entries for this ticker across all DataTypes., Every DataManager response is a DataResult.     data is None if and only if stat, SourceTag (+7 more)
...
Cohesion: 0.1
Nodes (31): Candlestick Chart, MACD (Daily) Chart, Price Chart with Bollinger Bands, RSI (14) Chart, Volume Chart, Chart Date Range: Nov 2025 to Apr 2026, Stock Search Feature, India Market (+23 more)

### Community 20 - "Community 20"
Cohesion: 0.12
Nodes (29): _clear_rate_limit_state(), get_batch_data(), get_intraday_chart_data(), get_live_price(), get_news(), get_price_data(), get_ticker_info(), _global_throttle() (+21 more)
...
Cohesion: 0.12
Nodes (21): Exception, is_commercial(), True if any waitlist row has converted_at set (first non-self subscriber).     F, Constraint Enforcer: commercial gate is fail-closed; SEBI rules are non-negotiab, Any exception in DB check → is_commercial() returns True (fail-closed: block yfi, Constraint Enforcer: commercial gate is fail-closed; SEBI rules are non-negotiab, If any waitlist row has converted_at set → commercial=True., Any exception in DB check → is_commercial() returns True (fail-closed: block yfi (+13 more)

### Community 29 - "Community 29"
Cohesion: 0.15
Nodes (21): Enum, concentration_pct(), HarvestOutput, HarvestResult, HarvestSubtype, load_portfolio_from_json(), OffsetOutput, OffsetResult (+13 more)
...
Cohesion: 0.16
Nodes (22): PATCH(), load_weights(), Return {signal_name: weight} for the given segment.      Queries signal_weights, Return {signal_name: weight} for the given segment.      Queries signal_weights, Buffett condition C4: DB weights below FUNDAMENTAL_WEIGHT_FLOOR raise ValueError, test_floor_enforced_at_emit(), _mock_client(), Phase 3 — weights.py tests. load_weights: returns DB active weights or COLD_STAR (+14 more)
...
Cohesion: 0.13
Nodes (20): get_current_regime(), Macro regime reader — as-of join against macro_regime table.  Returns the most r, As-of join: SELECT * FROM macro_regime WHERE regime_date <= :emitted_at     ORDE, is_stale(), _mock_client(), Phase 2 — regime.py tests. All tests use mocks: macro_regime has no seed data (D, Returns the most recent regime row where regime_date <= emitted_at., Must return None (not raise) when macro_regime is empty — Dalio's empty-table pa (+12 more)
...
Cohesion: 0.14
Nodes (12): _detect_circuit_flag(), parse_bhavcopy_nse(), NSE/BSE Bhavcopy download and parse.  Bhavcopy is NSE/BSE's daily end-of-day pri, Proxy circuit detection: H == L == C > 0 indicates a locked-price bar.      NSE, Proxy circuit detection: H == L == C > 0 indicates a locked-price bar.      NSE, Parse NSE Bhavcopy CSV text into OHLCV row dicts.      Only rows with SERIES in, Parse NSE Bhavcopy CSV text into OHLCV row dicts.      Only rows with SERIES in, Jhunjhunwala gate: proxy detects H==L==C (locked-price bar) as circuit. (+4 more)
...
Cohesion: 0.15
Nodes (18): _market_status(), _next_open(), Price + daily % + 5-day weekly % cards for all ticker bar instruments.     batch, BUY/WATCH/AVOID + RSI for each global instrument.     Fetches 3mo data (separate, Home page = Global Market Overview.     Tier 1 (immediate): morning brief + mark, Fixed 36px ticker strip via window.parent iframe injection.     get_batch_data c, _render_global_overview_prices(), _render_global_signals() (+10 more)

### Community 41 - "Community 41"
Cohesion: 0.11
Nodes (19): Phase 5 — Streamlit app presentation layer tests.  Design principle: pages expos, Weight review banner is returned when PROPOSED weights exist., No banner when there are no PROPOSED weights., No banner when there are no PROPOSED weights., Path page shows weight review banner when PROPOSED weights exist., When no ingest_status rows exist, data_viewer returns the MISSING banner., When no ingest_status rows exist, data_viewer returns the MISSING banner., Path page shows weight review banner when PROPOSED weights exist. (+11 more)
...
Cohesion: 0.11
Nodes (10): DataManager, Factory for UNAVAILABLE DataResult.     Use this everywhere — never construct Da, True if at least one source breaker is CLOSED and DataManager is not in bypass., Direct access to a source's circuit breaker. Used by source adapters (M4/M5)., Synchronous data fetch.  Returns DataResult — never raises.         M1: always r, Batch fetch.  Returns partial results — callers handle missing keys.         M1:, Atomically clears all cache layers for this ticker.         Called on stock swit, Full cache wipe.  Called on global Refresh button.         M2: clears L2 CacheMa (+2 more)
...
Cohesion: 0.13
Nodes (18): emit_pipeline(), Pure pipeline: no DB calls. Accepts pre-computed inputs.      Returns emit dict, Pure pipeline: no DB calls. Accepts pre-computed inputs.      Returns emit dict, Pure pipeline: no DB calls. Accepts pre-computed inputs.      Returns emit dict, Pure pipeline: no DB calls. Accepts pre-computed inputs.      Returns emit dict, Pure pipeline: no DB calls. Accepts pre-computed inputs.      Returns emit dict, Phase 3 — engine.py tests. Tests cover: calibrate_confidence (pure), emit_pipeli, SRA condition: emit_pipeline completes in ≤ 800ms under normal load.     Deferre (+10 more)
...
Cohesion: 0.11
Nodes (7): Phase 1 — constants.py tests. These pass immediately (constants.py is implemente, ROIC + FCF + pledge combined must be >= FUNDAMENTAL_WEIGHT_FLOOR for every class, C5: Pin ARBITRATION_MARGIN at 15.0 — any refactor moving it to config must fail, NG-4 / A2: web/lib/sebi-disclaimer.generated.ts must stay byte-identical to, test_arbitration_margin_pinned(), test_fundamental_floor_applies_to_all_cold_starts(), test_generated_ts_disclaimer_matches_source()

### Community 50 - "Community 50"
Cohesion: 0.11
Nodes (15): envelope(), SEBI envelope helper — wraps every API response.  Single source of truth for seb, Wrap data in the standard SEBI envelope.      Every endpoint must return this st, get_cold_start_label(), Return cold-start label when segment_obs < OBSERVATION_THRESHOLD, else None., Return cold-start label when segment_obs < OBSERVATION_THRESHOLD, else None., market_data(), GET /market-data — active instruments with latest OHLCV.  Two-query pattern: ins (+7 more)

### Community 51 - "Community 51"
Cohesion: 0.13
Nodes (19): GSI Dashboard Application v5.36, Home Screen Bottom Section, Latest Market News Section, Top Mover: AAPL (250.19, -3.25%), Top Mover: HINDUNILVR (2,110.60, +1.24%), Top Mover: INFY (1,339.40, +2.54%), Top Mover: TCS (2,539.80, +2.60%), Top Mover: TSLA (341.95, -3.08%) (+11 more)
...
Cohesion: 0.17
Nodes (14): compute_momentum_signal(), Pure momentum-signal computation (RF-I fix parts 1+2). No DB calls.      Calenda, _business_days(), _demo_ohlcv_rows(), Tests for scripts/backtest_replay.py — the Phase 0 backtest/refinement harness., Mirror case: close drops instead of rises -> BEAR, same magnitude., A longer, gently-varying series (60 business days) so the replay     produces mu, A weights_config that doesn't cover an emitted signal must raise, not     silent (+6 more)
...
Cohesion: 0.12
Nodes (15): Portfolio Kelly sizing and exit rules.  Kelly formula (Druckenmiller):   b = mag, E4: sector weight exceeds concentration cap., E1: position drifted outside Kelly band.      Triggers when |current - kelly_tar, E3: upside target has degraded below the minimum useful threshold., should_exit_e1(), should_exit_e3(), should_exit_e4(), E3: magnitude_target below 3% triggers exit. (+7 more)
...
Cohesion: 0.18
Nodes (9): HealthSnapshot, data_manager.py — Global Stock Intelligence Dashboard DataManager: resilient dat, Point-in-time health state.  Consumed by sidebar observability panel (M6).     R, Always safe to call — never raises.         Returns a snapshot of current health, validate(), _validate_batch(), _validate_info(), _validate_live() (+1 more)
...
Cohesion: 0.2
Nodes (14): _accuracy_context(), _capture_suppression_reason(), _historical_rows(), main(), _parse_args(), Call fn(*args, **kwargs), capturing the last WARNING logged by     `logger_name`, Load chronological, non-circuit-locked OHLCV rows for one instrument     within, Return (segment_obs, hit_rate, streak_count) — the same calibration     inputs e (+6 more)

### Community 65 - "Community 65"
Cohesion: 0.13
Nodes (15): calibrate_confidence(), Map raw confidence to calibrated probability p ∈ [0.0, 1.0].      Cold-start (se, Map raw confidence to calibrated probability p ∈ [0.0, 1.0].      Cold-start (se, Map raw confidence to calibrated probability p ∈ [0.0, 1.0].      Cold-start (se, Map raw confidence to calibrated probability p ∈ [0.0, 1.0].      Cold-start (se, Map raw confidence to calibrated probability p ∈ [0.0, 1.0].      Cold-start (se, test_cold_start_calibration_p_leq_confidence(), Cold-start: p = min(confidence/100, hit_rate). Reddy council condition. (+7 more)
...
Cohesion: 0.17
Nodes (9): _backtest_momentum_signal(), _CapturedSignal, _EngineClient, _live_momentum_signal(), _Query, Regression tests keeping backtest signal replay aligned with the live engine., Stop the live DB orchestration once its momentum signal is available., Replay and live paths must emit exactly the same direction and confidence. (+1 more)
...
Cohesion: 0.26
Nodes (7): hashToken(), isPortfolioSessionValid(), sessionValueForToken(), timingSafeStringEqual(), tokenMatchesConfigured(), loginToPortfolio(), logoutOfPortfolio()

### Community 75 - "Community 75"
Cohesion: 0.15
Nodes (5): _FakeClient, _InsertCapture, _Query, Fixed, in-memory Supabase stand-in — every table returns the same     canned row, Minimal insert-capable stand-in for bt_backtest_runs so dry_run=False     paths

### Community 76 - "Community 76"
Cohesion: 0.19
Nodes (11): approve_signal_weight(), Signal weight loader — DB active weights with cold-start fallback.  Priority (Mu, Validate DB-loaded weights before they enter the signal pipeline.      Raises Va, Change a signal weight row from PROPOSED → ACTIVE.      This is the sole applica, _validate_db_weights(), _mock_select_chain(), Phase 3 — approve_signal_weight() tests.  Governance rule: no signal_weights sta, Approval gate rejects any status that is not PROPOSED. (+3 more)

### Community 77 - "Community 77"
Cohesion: 0.18
Nodes (10): health(), GET /health — liveness + data presence gate.  ohlcv_rows > 0 is required for Ses, build_new_rows(), fetch_nifty500(), main(), Download and parse NSE's live Nifty 500 constituent CSV.      Returns list of di, Rows for tickers not already in instruments, ready for insert., get_supabase_client() (+2 more)

### Community 78 - "Community 78"
Cohesion: 0.17
Nodes (12): download_bhavcopy_nse(), Download NSE Bhavcopy CSV for the given YYYY-MM-DD date string.      Only called, Download NSE Bhavcopy CSV for the given YYYY-MM-DD date string.      Only called, Download NSE Bhavcopy CSV for the given YYYY-MM-DD date string.      Only called, Pre-flight gate (L2-B, Tarun's decision 2026-07-26): run     scripts/check_corpo, Return the last n_days NSE trading days up to and including end_date, oldest fir, Return the last n_days NSE trading days up to and including end_date, oldest fir, Backfill `trading_days` trading days of historical daily OHLCV for one instrumen (+4 more)
...
Cohesion: 0.18
Nodes (3): fleschKincaidGrade(), stripProperNouns(), lynchClassLexKey()

### Community 87 - "Community 87"
Cohesion: 0.17
Nodes (11): get_staleness_warning(), Return staleness warning when last_review_date is more than 90 days ago, else No, Return staleness warning when last_review_date is more than 90 days ago, else No, accuracy_state(), GET /accuracy — accuracy ledger summary.  Delegates to src/pages/accuracy.py for, 90-day overdue warning is shown on accuracy page., 90-day overdue warning is shown on accuracy page., No staleness warning when last review was recent. (+3 more)
...
Cohesion: 0.18
Nodes (11): get_disclaimer_html(), main(), AlphaVeda — Streamlit entry point.  SEBI disclaimer (Varghese): injected via get, Return the fixed-bottom SEBI disclaimer HTML block.      Always returns the full, Return the fixed-bottom SEBI disclaimer HTML block.      Always returns the full, Streamlit app entry point., Return the fixed-bottom SEBI disclaimer HTML block.      Always returns the full, Streamlit app entry point. (+3 more)

### Community 90 - "Community 90"
Cohesion: 0.23
Nodes (7): parse_bse_xbrl_fundamentals(), BSE XBRL fundamentals loader.  parse_bse_xbrl_fundamentals() is a pure transform, Parse a BSE XBRL fundamentals response dict into a normalised flat dict.      Re, Parse a BSE XBRL fundamentals response dict into a normalised flat dict.      Re, Parse a BSE XBRL fundamentals response dict into a normalised flat dict.      Re, _safe_float(), TestParseBseXbrlFundamentals

### Community 91 - "Community 91"
Cohesion: 0.3
Nodes (11): _expression_names(), Finding, _function_for(), _is_all_caps_module_assignment(), _looks_live(), main(), _name_terms(), _numeric_literals() (+3 more)
...
Cohesion: 0.25
Nodes (7): AccuracyPage(), daysBetween(), directionLexKey(), outcomeLexKey(), regimeLexKey(), segmentKey(), segmentObs()
...
Cohesion: 0.24
Nodes (7): _make_mock_supabase(), Phase 6 — Ingest pipeline unit tests (pure functions only, no live DB required)., Build a mock Supabase client whose .table().select()...execute() chain     retur, THE scenario that broke in production: an OK row already exists for         targ, Regression guard for the actual root cause: the query must use         .gte()/.l, Happy path: no prior OK row for target_date — ingest should proceed         (not, TestIngestIdempotencyGuard
...
Cohesion: 0.24
Nodes (3): toggleTaskStatus(), cycleStatus(), getServerSupabase()
...
Cohesion: 0.33
Nodes (6): LanguageToggle(), Lex(), ProbabilityFrame(), useLanguageMode(), lex(), naturalFrequency()

### Community 106 - "Community 106"
Cohesion: 0.25
Nodes (8): emit_signal(), Signal emit engine — full pipeline: arbitrate → discount → calibrate.  Pipeline, DB orchestrator: load context → build signal → emit → write accuracy_predictions, DB orchestrator: load context → build signal → emit → write accuracy_predictions, DB orchestrator: load context → build signal → emit → write accuracy_predictions, DB orchestrator: load context → build signal → emit → write accuracy_predictions, Return (lower, upper) calendar-date bounds equivalent to     +/- tolerance_tradi, _reference_tolerance_bounds()
...
Cohesion: 0.25
Nodes (3): GlossaryProvider(), useGlossary(), LanguageModeProvider()

### Community 112 - "Community 112"
Cohesion: 0.32
Nodes (5): get_ingest_staleness_flag(), Classify ingest freshness for the Imran SRA staleness banner.      Returns:, Classify ingest freshness for the Imran SRA staleness banner.      Returns:, Classify ingest freshness for the Imran SRA staleness banner.      Returns:, TestIngestStalenessFlag

### Community 113 - "Community 113"
Cohesion: 0.21
Nodes (8): b = 0.15/0.07 = 2.14; p=0.65, q=0.35, f = 0.65 - 0.35/2.14 = 0.486, ×0.25 = 0.12, b = 0.15/0.07 = 2.14; p=0.65, q=0.35, f = 0.65 - 0.35/2.14 = 0.486, ×0.25 = 0.12, b = 0.15/0.07 = 2.14; p=0.65, q=0.35, f = 0.65 - 0.35/2.14 = 0.486, ×0.25 = 0.12, Negative full Kelly (no edge) must return 0, not a floor., Negative full Kelly (no edge) must return 0, not a floor., Negative full Kelly (no edge) must return 0, not a floor., test_kelly_rupee_with_downside(), test_kelly_zero_on_negative_edge()

### Community 114 - "Community 114"
Cohesion: 0.29
Nodes (5): ABC, CommercialLicenseError, DataProvider, DataProvider ABC — every OHLCV data source must subclass this.  CommercialLicens, Raised when accessing a personal-use-only source under a commercial licence.
...
Cohesion: 0.33
Nodes (3): getLynchNarrative(), InstrumentPage(), startOfUtcWeek()

### Community 119 - "Community 119"
Cohesion: 0.33
Nodes (3): LexOrRaw(), isCommercial(), isPersonalContext()

### Community 120 - "Community 120"
Cohesion: 0.29
Nodes (5): Imran (SRA): stale or missing ingest_status rows must surface as visible warning, If last_run > 1 day ago → amber banner shown (Imran SRA condition)., Imran (SRA): stale or missing ingest_status rows must surface as visible warning, Full emit pipeline (arbitration → discount → calibrate → kelly) ≤ 800ms., TestImranConditions

### Community 121 - "Community 121"
Cohesion: 0.27
Nodes (6): get_rate_limit_state(), get_top_movers(), Return clean float Series from Close; guards MultiIndex yfinance output., Return top movers sorted by absolute % change.     Uses _yf_batch_download — ONE, Return rate-limit gate state for the observability dashboard.     No yfinance ca, _safe_close()

### Community 122 - "Community 122"
Cohesion: 0.33
Nodes (5): get_css(), AlphaVeda design system — CSS tokens, typography, component styles.  Colour pale, Return the full AlphaVeda CSS string for st.markdown injection., Render a signal card as an HTML string.      Args:         instrument: Stock sym, signal_card_html()

### Community 123 - "Community 123"
Cohesion: 0.53
Nodes (5): _fetch_active_instruments(), _fetch_closes(), find_discontinuities(), main(), Scan a ticker's date-sorted close series for |return| > threshold.      Rows wit
...
Cohesion: 0.4
Nodes (4): Reddy (calibration): cold-start calibration p must be ≤ confidence/100., When segment has <30 observations, p = min(confidence/100, hit_rate)., Reddy (calibration): cold-start calibration p must be ≤ confidence/100., TestReddyConditions
...
Cohesion: 0.4
Nodes (4): When some rows are circuit-locked, only non-circuit symbols resolve., When some rows are circuit-locked, only non-circuit symbols resolve., When some rows are circuit-locked, only non-circuit symbols resolve., When some rows are circuit-locked, only non-circuit symbols resolve.
...
Cohesion: 0.67
Nodes (3): main(), Codegen: alphaveda/constants.py SEBI_DISCLAIMER -> alphaveda/web/lib/sebi-discla, render()

### Community 137 - "Community 137"
Cohesion: 0.67
Nodes (3): main(), parse_migration_checks(), Return list of (kind, spec) tuples found in a migration file.

### Community 138 - "Community 138"
Cohesion: 0.67
Nodes (3): grandfathered_cost(), run(), SyntheticLot
...
## Knowledge Gaps
- **1020 isolated node(s):** `R-ZIP · KI-014: Re-read packaged zip from disk and run full suite.     Catches f`, `Return the forecast history dict.     Primary store: st.session_state[_SS_KEY].`, `Persist the forecast history dict.     Always writes to session_state (works on`, `Record a new forecast entry (one per ticker per day).     simulation: optional d`, `Check all pending forecasts whose due date has passed and mark them     resolved` (+1015 more)
...
- **Why does `get_supabase_client()` connect `Community 77` to `Community 64`, `Community 33`, `Community 2`, `Community 96`, `Community 72`, `Community 106`, `Community 76`, `Community 78`, `Community 79`, `Community 80`, `Community 60`, `Community 50`, `Community 87`, `Community 88`, `Community 123`, `Community 28`, `Community 95`, `Community 31`?**
  _High betweenness centrality (0.093) - this node is a cross-community bridge._
- **Why does `Exception` connect `Community 28` to `Community 33`, `Community 67`, `Community 114`, `Community 20`, `Community 56`, `Community 31`?**
  _High betweenness centrality (0.079) - this node is a cross-community bridge._
- **Why does `_set_rate_limited()` connect `Community 20` to `Community 28`?**
...
- **Are the 22 inferred relationships involving `PATCH()` (e.g. with `getServerSupabase()` and `test_floor_enforced_at_emit()`) actually correct?**
  _`PATCH()` has 22 INFERRED edges - model-reasoned connections that need verification._
- **Are the 20 inferred relationships involving `get_supabase_client()` (e.g. with `supabase_client()` and `run_ingest()`) actually correct?**
  _`get_supabase_client()` has 20 INFERRED edges - model-reasoned connections that need verification._