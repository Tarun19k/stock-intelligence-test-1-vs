# Graph Report - stock-intelligence-test-1-vs  (2026-07-21)

## Corpus Check
- 163 files · ~1,439,495 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2431 nodes · 3575 edges · 174 communities detected
- Extraction: 87% EXTRACTED · 13% INFERRED · 0% AMBIGUOUS · INFERRED: 468 edges (avg confidence: 0.79)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `7303f38e`
...
7. `get_supabase_client()` - 20 edges
8. `derive_cycle_phase()` - 20 edges
9. `ok()` - 19 edges
10. `Sensex Index (211)` - 19 edges
...
Cohesion: 0.09
Nodes (54): Bug: Static Text with Delay Load, Crude Oil WTI ($115), Crude WTI, Gold ($4,685), Stock Search Feature, USD/INR: 92.85 (+0.13%), USD/INR Exchange Rate, USD/INR (+46 more)
...
Cohesion: 0.06
Nodes (22): LanguageToggle(), Lex(), LexOrRaw(), ProbabilityFrame(), fleschKincaidGrade(), stripProperNouns(), isCommercial(), isPersonalContext() (+14 more)
...
Cohesion: 0.14
Nodes (21): load_weights(), Return {signal_name: weight} for the given segment.      Queries signal_weights, Return {signal_name: weight} for the given segment.      Queries signal_weights, Buffett condition C4: DB weights below FUNDAMENTAL_WEIGHT_FLOOR raise ValueError, test_floor_enforced_at_emit(), _mock_client(), Phase 3 — weights.py tests. load_weights: returns DB active weights or COLD_STAR, Empty DB result → COLD_START_WEIGHTS for the given segment. (+13 more)
...
Cohesion: 0.12
Nodes (13): envelope(), SEBI envelope helper — wraps every API response.  Single source of truth for seb, Wrap data in the standard SEBI envelope.      Every endpoint must return this st, accuracy_state(), GET /accuracy — accuracy ledger summary.  Delegates to src/pages/accuracy.py for, health(), GET /health — liveness + data presence gate.  ohlcv_rows > 0 is required for Ses, market_data() (+5 more)
...
Cohesion: 0.14
Nodes (17): emit_pipeline(), Pure pipeline: no DB calls. Accepts pre-computed inputs.      Returns emit dict, Pure pipeline: no DB calls. Accepts pre-computed inputs.      Returns emit dict, Pure pipeline: no DB calls. Accepts pre-computed inputs.      Returns emit dict, Pure pipeline: no DB calls. Accepts pre-computed inputs.      Returns emit dict, Phase 3 — engine.py tests. Tests cover: calibrate_confidence (pure), emit_pipeli, SRA condition: emit_pipeline completes in ≤ 800ms under normal load.     Deferre, Suppressed arbitration → None from emit_pipeline. (+9 more)
...
Cohesion: 0.14
Nodes (14): calibrate_confidence(), Map raw confidence to calibrated probability p ∈ [0.0, 1.0].      Cold-start (se, Map raw confidence to calibrated probability p ∈ [0.0, 1.0].      Cold-start (se, Map raw confidence to calibrated probability p ∈ [0.0, 1.0].      Cold-start (se, Map raw confidence to calibrated probability p ∈ [0.0, 1.0].      Cold-start (se, test_cold_start_calibration_p_leq_confidence(), Cold-start: p = min(confidence/100, hit_rate). Reddy council condition., Calibrated p must never exceed confidence/100 in cold-start. (+6 more)

### Community 66 - "Community 66"
Cohesion: 0.19
Nodes (12): _business_days(), _demo_ohlcv_rows(), Tests for scripts/backtest_replay.py — the Phase 0 backtest/refinement harness., Mirror case: close drops instead of rises -> BEAR, same magnitude., A longer, gently-varying series (60 business days) so the replay     produces mu, A weights_config that doesn't cover an emitted signal must raise, not     silent, A single OHLCV row (no history 21 days back) must suppress via     RF-I_STALE_RE, test_backtest_replay_determinism() (+4 more)
...
Cohesion: 0.17
Nodes (11): get_suppression_label(), Return the rupee-suppression state label — deliberate state, not degraded fallba, Streamlit render entry point — called by app.py navigation., Return the rupee-suppression state label — deliberate state, not degraded fallba, Streamlit render entry point — called by app.py navigation., render(), path_state(), GET /path — Kelly sizing state.  Delegates to src/pages/path.py for commercial g (+3 more)
...
Cohesion: 0.27
Nodes (9): compute_momentum_signal(), emit_signal(), Signal emit engine — full pipeline: arbitrate → discount → calibrate.  Pipeline, Pure momentum-signal computation (RF-I fix parts 1+2). No DB calls.      Calenda, DB orchestrator: load context → build signal → emit → write accuracy_predictions, DB orchestrator: load context → build signal → emit → write accuracy_predictions, DB orchestrator: load context → build signal → emit → write accuracy_predictions, Return (lower, upper) calendar-date bounds equivalent to     +/- tolerance_tradi (+1 more)
...
Cohesion: 0.25
Nodes (8): download_bhavcopy_nse(), Download NSE Bhavcopy CSV for the given YYYY-MM-DD date string.      Only called, Download NSE Bhavcopy CSV for the given YYYY-MM-DD date string.      Only called, Download NSE Bhavcopy CSV for the given YYYY-MM-DD date string.      Only called, Return the last n_days NSE trading days up to and including end_date, oldest fir, Backfill `trading_days` trading days of historical daily OHLCV for one instrumen, run_backfill(), _trading_days()
...
Cohesion: 0.21
Nodes (8): Even with extreme edge, result must not exceed MAX_POSITION_PCT × portfolio., Even with extreme edge, result must not exceed MAX_POSITION_PCT × portfolio., Even with extreme edge, result must not exceed MAX_POSITION_PCT × portfolio., Negative full Kelly (no edge) must return 0, not a floor., Negative full Kelly (no edge) must return 0, not a floor., Negative full Kelly (no edge) must return 0, not a floor., test_kelly_capped_at_max_position(), test_kelly_zero_on_negative_edge()

### Community 108 - "Community 108"
Cohesion: 0.25
Nodes (8): _clear_rate_limit_state(), get_batch_data(), _parse_batch_raw(), Reset consecutive hit counter after a fully clean batch. No global needed., Extract per-ticker DataFrames from a yf.download() grouped result., Chunked batch downloader with global cooldown gate (v5.27).      v5.27 changes:, Unified cached batch fetch — replaces get_ticker_bar_data + get_group_data., _yf_batch_download()
...
Cohesion: 0.38
Nodes (6): get_proposed_weights_count(), get_weight_review_banner(), Path page — Kelly sizing and rupee display.  Constraint Enforcer condition: rupe, Delegate to signals.get_proposed_weights_count — path page shares the same banne, Delegate to signals.get_proposed_weights_count — path page shares the same banne, Return weight review banner if PROPOSED weights exist (mirrors signals page).

### Community 114 - "Community 114"
Cohesion: 0.33
Nodes (6): CommercialLicenseError, Raised when accessing a personal-use-only source under a commercial licence., Exception, _CapturedSignal, Stop the live DB orchestration once its momentum signal is available., test_is_commercial_fail_closed()
...
Cohesion: 0.33
Nodes (6): get_cold_start_label(), Return cold-start label when segment_obs < OBSERVATION_THRESHOLD, else None., Return cold-start label when segment_obs < OBSERVATION_THRESHOLD, else None., Cold-start label is returned when segment_obs < OBSERVATION_THRESHOLD., Cold-start label is returned when segment_obs < OBSERVATION_THRESHOLD., test_cold_start_label_visible()
...
## Knowledge Gaps
- **966 isolated node(s):** `R-ZIP · KI-014: Re-read packaged zip from disk and run full suite.     Catches f`, `Return the forecast history dict.     Primary store: st.session_state[_SS_KEY].`, `Persist the forecast history dict.     Always writes to session_state (works on`, `Record a new forecast entry (one per ticker per day).     simulation: optional d`, `Check all pending forecasts whose due date has passed and mark them     resolved` (+961 more)
...
- **Why does `get_supabase_client()` connect `Community 44` to `Community 98`, `Community 67`, `Community 70`, `Community 72`, `Community 73`, `Community 29`, `Community 83`, `Community 54`, `Community 86`, `Community 87`, `Community 59`, `Community 61`, `Community 30`, `Community 95`?**
  _High betweenness centrality (0.075) - this node is a cross-community bridge._
- **Why does `Exception` connect `Community 114` to `Community 32`, `Community 99`, `Community 53`, `Community 59`, `Community 29`, `Community 30`?**
  _High betweenness centrality (0.066) - this node is a cross-community bridge._
- **Why does `_set_rate_limited()` connect `Community 32` to `Community 114`, `Community 108`?**
