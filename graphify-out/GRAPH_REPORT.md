# Graph Report - stock-intelligence-test-1-vs  (2026-06-28)

## Corpus Check
- 119 files · ~1,368,629 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2166 nodes · 3231 edges · 158 communities detected
- Extraction: 87% EXTRACTED · 13% INFERRED · 0% AMBIGUOUS · INFERRED: 428 edges (avg confidence: 0.78)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `2530b2b7`
...
Cohesion: 0.03
Nodes (61): Refresh Data Button, App Version v5.36, Refresh Data Button, Commodities & Global Inflation, Compare Tab, Open Dashboard Button CTA, Start Analysing Free Right Now CTA, Dark Theme UI (+53 more)
...
Cohesion: 0.1
Nodes (49): GSI Dashboard v5.36, Bug: Static Text with Delay Load, Crude Oil WTI ($115), Crude WTI, Gold ($4,685), USD/INR: 92.85 (+0.13%), USD/INR Exchange Rate, USD/INR (+41 more)
...
Cohesion: 0.08
Nodes (32): All Risks Mitigated Status Banner, App Internals Section, App Version v5.38, Deploy Button, Compliance Gates (Live Check), Baseline Metric (437), Sprint Progress Metric (7/9), Sprint Status Metric (COMPLETE) (+24 more)
...
Cohesion: 0.07
Nodes (30): Refresh Data Button, Group-Level Sector Selection Feature, Market Filter (India), Search Filter (Company Name or Symbol), Auto & EV Sector Group, Banks & Finance Sector Group, FMCG & Consumer Sector Group, IT & Technology Sector Group (+22 more)
...
Cohesion: 0.13
Nodes (25): get_intraday_chart_data(), get_live_price(), get_news(), get_price_data(), get_ticker_info(), get_top_movers(), _global_throttle(), _is_allowed_rss() (+17 more)
...
Cohesion: 0.14
Nodes (21): Multi-Asset Weekly Performance Bar Chart, Multi-Asset Weekly Return Bar Chart, Week of 06 Apr – 07 Apr 2026 (current week), Current Week Market View, Crude Oil WTI Card ($115), Gold Index Card (4,685), Hang Seng Index Card (HK$25,117), NASDAQ 100 Index Card (24,060) (+13 more)
...
Cohesion: 0.18
Nodes (16): Estimated Tokens Column, Sub-sprint Column, compliance_check.py File, .claude/commands/new-session File, regression.py File, .claude/commands/sprint-monitor File, Sequential Execution Mode, Haiku Model (+8 more)
...
Cohesion: 0.18
Nodes (12): download_bhavcopy_nse(), Download NSE Bhavcopy CSV for the given YYYY-MM-DD date string.      Only called, Download NSE Bhavcopy CSV for the given YYYY-MM-DD date string.      Only called, Download NSE Bhavcopy CSV for the given YYYY-MM-DD date string.      Only called, _is_trading_day(), Return True if d is an NSE trading day (exchange-calendar aware).      Falls bac, Return True if d is an NSE trading day (exchange-calendar aware).      Falls bac, Run the full ingest pipeline for target_date (defaults to today).      Returns a (+4 more)
...
Cohesion: 0.18
Nodes (12): get_cold_start_label(), get_proposed_weights_count(), get_weight_review_banner(), Signals page — signal output with cold-start label and weight review banner.  Ta, Return cold-start label when segment_obs < OBSERVATION_THRESHOLD, else None., Return count of signal_weights rows in PROPOSED status., Return review banner when PROPOSED weights exist, else None., Streamlit render entry point — called by app.py navigation. (+4 more)
...
Cohesion: 0.18
Nodes (10): calibrate_confidence(), Signal emit engine — full pipeline: arbitrate → discount → calibrate.  Pipeline, Map raw confidence to calibrated probability p ∈ [0.0, 1.0].      Cold-start (se, test_cold_start_calibration_p_leq_confidence(), Cold-start: p = min(confidence/100, hit_rate). Reddy council condition., Warm path (>= OBSERVATION_THRESHOLD): p = hit_rate (capped at confidence/100)., Calibrated p must always be in [0.0, 1.0]., test_calibration_result_in_unit_interval() (+2 more)
...
Cohesion: 0.22
Nodes (9): Breadcrumb: Current Week > India > Nifty 50, Weekly Returns All Stocks Bar Chart, Sidebar Stock Selector, No Stock Selected State (showing weekly summary), Portfolio Allocator Tab, Signal Summary Tab, Weekly Returns Tab, UI State: No Stock Selected (showing weekly summary) (+1 more)
...
Cohesion: 0.21
Nodes (8): If downside_target is None, return 0 — no rupee amount on Path page., Even with extreme edge, result must not exceed MAX_POSITION_PCT × portfolio., Even with extreme edge, result must not exceed MAX_POSITION_PCT × portfolio., Even with extreme edge, result must not exceed MAX_POSITION_PCT × portfolio., If downside_target is None, return 0 — no rupee amount on Path page., If downside_target is None, return 0 — no rupee amount on Path page., test_kelly_capped_at_max_position(), test_kelly_no_rupee_without_downside()
...
Cohesion: 0.5
Nodes (3): get_supabase_client(), Supabase client singleton + commercial gate.  get_supabase_client(): module-leve, Return the module-level Supabase client, creating it once per process.
...
## Knowledge Gaps
- **882 isolated node(s):** `R-ZIP · KI-014: Re-read packaged zip from disk and run full suite.     Catches f`, `Return the forecast history dict.     Primary store: st.session_state[_SS_KEY].`, `Persist the forecast history dict.     Always writes to session_state (works on`, `Record a new forecast entry (one per ticker per day).     simulation: optional d`, `Check all pending forecasts whose due date has passed and mark them     resolved` (+877 more)
...
- **Why does `Exception` connect `Community 57` to `Community 96`, `Community 34`, `Community 40`, `Community 47`, `Community 86`, `Community 93`?**
  _High betweenness centrality (0.086) - this node is a cross-community bridge._
- **Why does `test_is_commercial_fail_closed()` connect `Community 57` to `Community 25`?**
  _High betweenness centrality (0.063) - this node is a cross-community bridge._
- **Why does `get_supabase_client()` connect `Community 116` to `Community 64`, `Community 65`, `Community 34`, `Community 67`, `Community 40`, `Community 73`, `Community 83`, `Community 57`, `Community 63`?**
