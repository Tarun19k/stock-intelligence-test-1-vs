...
Cohesion: 0.07
Nodes (36): Breadcrumb: Current Week > India > Nifty 50, Refresh Data Button, Weekly Returns All Stocks Bar Chart, Stock Search Feature, Market Filter (India), Search Filter (Company Name or Symbol), Nifty 50 Group, India Market (+28 more)
...
Cohesion: 0.17
Nodes (10): CacheManager, DataContract, Bounded LRU in-memory cache (L2).      Deployment note: Streamlit-specific.  Sin, Wire-level shape validator for each DataType.      Validates that data received, _make_result(), TestCacheManagerHit, TestCacheManagerInvalidate, TestCacheManagerLRU (+2 more)
...
Cohesion: 0.12
Nodes (19): calibrate_confidence(), Map raw confidence to calibrated probability p ∈ [0.0, 1.0].      Cold-start (se, Map raw confidence to calibrated probability p ∈ [0.0, 1.0].      Cold-start (se, test_cold_start_calibration_p_leq_confidence(), Phase 3 — engine.py tests. Tests cover: calibrate_confidence (pure), emit_pipeli, Buffett condition C4: DB weights below FUNDAMENTAL_WEIGHT_FLOOR raise ValueError, Cold-start: p = min(confidence/100, hit_rate). Reddy council condition., Calibrated p must never exceed confidence/100 in cold-start. (+11 more)
...
Cohesion: 0.15
Nodes (13): emit_pipeline(), emit_signal(), Signal emit engine — full pipeline: arbitrate → discount → calibrate.  Pipeline, Pure pipeline: no DB calls. Accepts pre-computed inputs.      Returns emit dict, Pure pipeline: no DB calls. Accepts pre-computed inputs.      Returns emit dict, DB orchestrator: load context → build signal → emit → write accuracy_predictions, SRA condition: emit_pipeline completes in ≤ 800ms under normal load.     Deferre, Suppressed arbitration → None from emit_pipeline. (+5 more)
...
- **Why does `Exception` connect `Community 7` to `Community 4`, `Community 29`, `Community 36`?**
  _High betweenness centrality (0.071) - this node is a cross-community bridge._
- **Why does `get_supabase_client()` connect `Community 47` to `Community 64`, `Community 36`, `Community 70`, `Community 7`, `Community 73`, `Community 29`, `Community 83`, `Community 86`, `Community 91`, `Community 92`, `Community 61`?**
