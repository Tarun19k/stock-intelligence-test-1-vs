# Graph Report - stock-intelligence-test-1-vs  (2026-07-02)

## Corpus Check
- 144 files · ~1,377,688 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2240 nodes · 3309 edges · 161 communities detected
- Extraction: 86% EXTRACTED · 14% INFERRED · 0% AMBIGUOUS · INFERRED: 448 edges (avg confidence: 0.78)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `e0f0daa4`
...
Cohesion: 0.14
Nodes (12): CacheManager, DataContract, Bounded LRU in-memory cache (L2).      Deployment note: Streamlit-specific.  Sin, Store result.  No-op if status == UNAVAILABLE.         Evicts the LRU entry if a, Remove all cache entries for this ticker across all DataTypes., Wire-level shape validator for each DataType.      Validates that data received, _make_result(), TestCacheManagerHit (+4 more)
...
Cohesion: 0.12
Nodes (20): emit_pipeline(), emit_signal(), Signal emit engine — full pipeline: arbitrate → discount → calibrate.  Pipeline, Pure pipeline: no DB calls. Accepts pre-computed inputs.      Returns emit dict, Pure pipeline: no DB calls. Accepts pre-computed inputs.      Returns emit dict, DB orchestrator: load context → build signal → emit → write accuracy_predictions, Phase 3 — engine.py tests. Tests cover: calibrate_confidence (pure), emit_pipeli, Buffett condition C4: DB weights below FUNDAMENTAL_WEIGHT_FLOOR raise ValueError (+12 more)
...
Cohesion: 0.16
Nodes (17): _clear_rate_limit_state(), get_batch_data(), get_news(), get_top_movers(), _is_allowed_rss(), _parse_batch_raw(), Reset consecutive hit counter after a fully clean batch. No global needed., Extract per-ticker DataFrames from a yf.download() grouped result. (+9 more)
...
Cohesion: 0.15
Nodes (13): health(), GET /health — liveness + data presence gate.  ohlcv_rows > 0 is required for Ses, approve_signal_weight(), Change a signal weight row from PROPOSED → ACTIVE.      This is the sole applica, get_supabase_client(), Supabase client singleton + commercial gate.  get_supabase_client(): module-leve, Return the module-level Supabase client, creating it once per process., _mock_select_chain() (+5 more)
...
Cohesion: 0.24
Nodes (12): get_intraday_chart_data(), get_live_price(), get_price_data(), _global_throttle(), _normalize_df(), Token-bucket throttle. Fast for small bursts, paced for sustained use., Single-ticker download with global cooldown check (v5.27).     If _is_rate_limit, Guarantee df always has string columns: Open High Low Close Volume.     Handles (+4 more)
...
Cohesion: 0.18
Nodes (10): E1: position drifted outside Kelly band.      Triggers when |current - kelly_tar, should_exit_e1(), b = 0.15/0.07 = 2.14; p=0.65, q=0.35, f = 0.65 - 0.35/2.14 = 0.486, ×0.25 = 0.12, b = 0.15/0.07 = 2.14; p=0.65, q=0.35, f = 0.65 - 0.35/2.14 = 0.486, ×0.25 = 0.12, b = 0.15/0.07 = 2.14; p=0.65, q=0.35, f = 0.65 - 0.35/2.14 = 0.486, ×0.25 = 0.12, E1: position that has drifted 60% away from Kelly target triggers exit., E1: position within tolerance band must not trigger exit., test_exit_e1() (+2 more)
...
Cohesion: 0.5
Nodes (4): get_ticker_info(), _is_rate_limited(), Fetch metadata/fundamentals for a ticker.     cache_buster: same semantics as ge, True if inside a 429 cooldown window. Fast path — no lock needed.
...
- **Why does `Exception` connect `Community 89` to `Community 38`, `Community 45`, `Community 53`, `Community 92`, `Community 30`, `Community 63`?**
  _High betweenness centrality (0.080) - this node is a cross-community bridge._
- **Why does `get_supabase_client()` connect `Community 52` to `Community 97`, `Community 99`, `Community 38`, `Community 71`, `Community 76`, `Community 30`, `Community 86`, `Community 87`, `Community 31`, `Community 62`, `Community 63`?**
  _High betweenness centrality (0.070) - this node is a cross-community bridge._
- **Why does `_set_rate_limited()` connect `Community 45` to `Community 89`, `Community 73`?**
